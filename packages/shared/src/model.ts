...
/**
 * Re-exported rather than declared here. The phase vocabulary belongs to the
 * request contract (@lifecycle/schemas), which the operator console imports
 * without dragging in the Firestore and Secret Manager clients this package
 * needs. One definition, and the dependency points from storage to contract.
 */
export type { Phase } from '@lifecycle/schemas';
...
/**
 * Every request status, as a value rather than only a type. The API validates
 * a status filter against this, so an unknown one is a 400 rather than a query
 * that quietly matches nothing (REQ-011 AC-4).
 */
 ...
 */
export const NON_TERMINAL_REQUEST_STATUSES: readonly RequestStatus[] = REQUEST_STATUSES.filter(
  (s) => !TERMINAL_REQUEST_STATUSES.includes(s),
);

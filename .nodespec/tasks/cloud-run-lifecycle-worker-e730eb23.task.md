# Task: Cloud Run: lifecycle-worker

> **Scope:** implement ONLY this node ("Cloud Run: lifecycle-worker"). Work belonging to other nodes appears here solely as interfaces and coordination points — do not implement or re-derive it.
...
### REQ-026: Cloud Run lifecycle-worker service provisioning
Category: technical | Status: in-progress
Owned by the Cloud Run: lifecycle-worker node. Provisions the step-executor service: container image, its own runtime service account — the only identity carrying the Workspace admin role — ingress restricted so only in-project callers can reach it, scale-to-zero, and a request timeout long enough for the slowest Workspace step. This service is not attached to the load balancer and is therefore not IAP-protected, which is deliberate and documented in REQ-007: it has no human ingress path at all.

...
- [x] No application runtime service account holds permission to delete log entries or shorten the bucket's retention policy, verified against the deployed IAM policy
  → possible match: Contract "Audit Log Sink" (dependency) from Lifecycle Step Executor (unverified — requirement not mapped to that node)
